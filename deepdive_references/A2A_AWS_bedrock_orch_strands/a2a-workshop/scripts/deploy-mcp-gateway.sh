...
#############################################################################
# A2A Workshop - MCP Gateway Deployment Script (3-Step Workflow)
#
# This script deploys the MCP Gateway using a 3-step process:
#   Step 1: Deploy resources stack (ECR + CloudWatch Logs)
#   Step 2: Build and push Docker image to ECR
#   Step 3: Deploy service stack (ECS + ALB)
#
...
#   ./deploy-mcp-gateway.sh --delete    # Delete both stacks
#
#############################################################################
...
Deploys the MCP Gateway (with ALB) to AWS ECS Fargate using a 3-step workflow.

Options:
  -h, --help              Show this help message
  -d, --delete            Delete MCP Gateway stacks (both resources and service)
  -l, --log-level LEVEL   Set log level (DEBUG, INFO, WARNING, ERROR) [default: INFO]

Examples:
  $0                      # Deploy MCP Gateway (3-step workflow)
  $0 --log-level DEBUG    # Deploy with DEBUG logging
  $0 --delete             # Delete MCP Gateway stacks
...
get_code_server_ip() {
    print_info "Getting code-server IP for ALB security..." >&2

    # Find code-server stack
    local stack_name=$(aws cloudformation describe-stacks \
        --query 'Stacks[?contains(StackName, `CodeServer`) || contains(StackName, `code-server`)].StackName' \
        --output text 2>/dev/null | head -n1)

    if [ -z "$stack_name" ]; then
        print_error "Code-server stack not found"
        exit 1
    fi

    # Get InstanceId from stack outputs
    ...
    if [ -z "$instance_id" ]; then
        print_error "Could not get InstanceId from code-server stack"
        exit 1
    fi
    ...
    if [ -z "$public_ip" ] || [ "$public_ip" = "None" ]; then
        print_error "Could not get public IP for instance $instance_id"
        exit 1
    fi

    print_success "Code-server IP: $public_ip" >&2
    echo "$public_ip"
}
...
deploy_resources_stack() {
    ...
    RESOURCES_STACK=$(aws cloudformation describe-stacks \
        --query "Stacks[?contains(StackName, 'a2a-workshop') && contains(StackName, 'MCPGateway') && contains(StackName, 'Resources')].StackName" \
        --output text 2>/dev/null | head -n1)

    if [ -z "$RESOURCES_STACK" ]; then
        print_error "Workshop resources stack not found"
        echo "Expected pattern: a2a-workshop-*MCPGateway*Resources*"
        echo "Available stacks:"
        aws cloudformation list-stacks \
            --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE \
            --query 'StackSummaries[?contains(StackName, `MCPGateway`)].StackName' \
            --output text
        exit 1
    fi
...
deploy_service_stack() {
    ...
    SERVICE_STACK=$(aws cloudformation describe-stacks \
        --query "Stacks[?contains(StackName, 'a2a-workshop') && contains(StackName, 'MCPGateway') && contains(StackName, 'Service')].StackName" \
        --output text 2>/dev/null | head -n1)

    if [ -z "$SERVICE_STACK" ]; then
        print_error "Workshop service stack not found"
        echo "Expected pattern: a2a-workshop-*MCPGateway*Service*"
        echo "Available stacks:"
        aws cloudformation list-stacks \
            --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE \
            --query 'StackSummaries[?contains(StackName, `MCPGateway`)].StackName' \
...
delete_stacks() {
    print_header "Delete MCP Gateway Stacks"
    ...
    print_success "MCP Gateway stacks deleted"
}
...
show_deployment_info() {
    ...
    local current_cidr=$(aws cloudformation describe-stacks \
        --stack-name "$SERVICE_STACK" \
        --query 'Stacks[0].Parameters[?ParameterKey==`AllowedCIDR`].ParameterValue' \
        --output text 2>/dev/null || echo "N/A")
    echo "  ALB Access Restricted to: $current_cidr"
    if [ "$current_cidr" != "0.0.0.0/0" ]; then
        print_success "ALB is secured - only code-server can access"
    fi
    echo ""
    ...
    print_success "MCP Gateway URL for Claude Code:"
    echo "  ${gateway_url}"
    echo ""
    ...
    print_success "MCP Gateway deployed successfully!"
    echo ""
    print_info "Next steps:"
    echo "  1. Deploy Orchestrator"
    echo "  2. Configure Claude Code with the Gateway URL"
    echo "  3. Deploy specialist agents"
}
...
main() {
    ...
    echo -e "${BLUE}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║                                                                   ║${NC}"
    echo -e "${BLUE}║        A2A Workshop - MCP Gateway Deployment (3-Step Workflow)    ║${NC}"
    echo -e "${BLUE}║                                                                   ║${NC}"
    echo -e "${BLUE}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    if [ "$delete_mode" = true ]; then
        ...
        # Get code-server IP for ALB security
        CODE_SERVER_IP=$(get_code_server_ip)
        ALLOWED_CIDR="${CODE_SERVER_IP}/32"
        echo ""
        ...
        deploy_service_stack "$ALLOWED_CIDR"
        echo ""
