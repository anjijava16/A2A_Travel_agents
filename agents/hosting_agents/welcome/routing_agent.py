...
class RoutingAgent:
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        agent_names = list(self.remote_agent_connections.keys())
        
        # Map agents (you might need to adjust this based on your actual agent names)
        policy_agent = next((name for name in agent_names if "policy" in name.lower()), agent_names[0] if agent_names else "PolicyAgent")
        research_agent = next((name for name in agent_names if "research" in name.lower()), agent_names[1] if len(agent_names) > 1 else "ResearchAgent")
        provider_agent = next((name for name in agent_names if "provider" in name.lower()), agent_names[2] if len(agent_names) > 2 else "ProviderAgent")
        ...
        **Core Workflow - MUST FOLLOW FOR EVERY QUERY:**
        1. **First**: Use `send_message` with agent_name="{policy_agent}" for insurance analysis
        2. **Second**: Use `send_message` with agent_name="{research_agent}" for medical assessment  
        3. **Third**: If user provides location, use `send_message` with agent_name="{provider_agent}" for provider recommendations
        4. **Finally**: Synthesize all responses into comprehensive guidance

        **CRITICAL RULE FOR LOCATION:**
        - If the user DOES NOT provide location in their initial query, you MUST ask for it BEFORE calling {provider_agent}
        - Ask for location ONCE at the beginning if needed
        - Only call {provider_agent} AFTER you have the location information
        ...
        **Example Query Handling:**
        ...
        Your process:
        1. Ask: "Please provide your location so I can find nearby providers for you."
        2. Once location received: `send_message(agent_name="{policy_agent}", task="What does Blue Cross insurance cover for abdominal pain evaluation and treatment?")`
        3. `send_message(agent_name="{research_agent}", task="What are potential causes, urgency levels, and treatments for abdominal pain?")`
        4. `send_message(agent_name="{provider_agent}", task="Find gastroenterologists or urgent care for abdominal pain evaluation in [user_location]")`
        5. Synthesize all responses

        **Important Rules:**
        - ALWAYS consult ALL THREE agents for comprehensive healthcare queries
        - If user doesn't provide location, ASK FIRST before proceeding
        - Present information clearly with sections
        """
    def root_instruction_working(self, context: ReadonlyContext) -> str:
    ...
    async def send_message(
        ...
        """
        print(f"\n🔗 CALLING REMOTE AGENT: {agent_name}")
        print(f"📝 TASK: {task}")
...
async def test_orchestration():
    ...
    test_cases = [
        {
            "query": "I have abdominal pain and Blue Cross insurance — what's covered and who should I see nearby in San Francisco, CA?",
    ...
    for i, test in enumerate(test_cases):
        ...
        # Collect all events - use regular for loop, not async for
        events = []
        try:
            # Run returns a regular generator, not async
            for event in ROUTING_AGENT_RUNNER.run(
                user_id=USER_ID, 
            ...
            import traceback
            traceback.print_exc()
            continue  # Skip to next test case if this one fails
        
        # Process collected events
        for event in events:
            print(f"\n📡 EVENT from: {event.author}")
            
            # Safely check for tool_invocation
            if hasattr(event, 'tool_invocation') and event.tool_invocation:
                print(f"🔧 TOOL INVOCATION: {event.tool_invocation.name}")
                if event.tool_invocation.name == "send_message":
                    agent_name = event.tool_invocation.args.get("agent_name", "unknown")
                    task = event.tool_invocation.args.get("task", "unknown")
                    print(f"📞 CALLING A2A AGENT: {agent_name}")
                    print(f"📝 TASK: {task}")
            ...
            if event.is_final_response() and event.content:
                ...
                print(final_answer)
                print("\n" + "="*60)
                print("📊 SUMMARY: Successfully orchestrated healthcare query!")
async def test_orchestration_error():
    ...
    for i, test in enumerate(test_cases):
        ...
        try:
            ...
            events = [event async for event in ROUTING_AGENT_RUNNER.run(
                user_id=USER_ID, 
                session_id=SESSION_ID, 
                new_message=content
            )]
...
async def test_orchestration_error():
    ...
    for i, test in enumerate(test_cases):
        ...
        agent_calls = []  # Track which agents were actually called
        ...
        for event in ROUTING_AGENT_RUNNER.run(user_id=USER_ID, session_id=SESSION_ID, new_message=content):
            ...
            if event.tool_invocation:
                print(f"🔧 TOOL INVOCATION: {event.tool_invocation.name}")
                print(f"🔧 TOOL ARGUMENTS: {event.tool_invocation.args}")
                if event.tool_invocation.name == "send_message":
                    agent_name = event.tool_invocation.args.get("agent_name", "unknown")
            ...
            if event.is_final_response() and event.content:
                ...
                print("📊 SUMMARY")
                print("="*60)
                print(f"Total A2A agent calls: {len(agent_calls)}")
                print(f"Agents called: {', '.join(agent_calls)}")
    # for i, test in enumerate(test_cases):
    #     print(f"\n{'=' * 60}")
    #     print(f"Test {i + 1}: {test['query']}")
    #     print(f"Expected to use: {', '.join(test['expected_agents'])}")
